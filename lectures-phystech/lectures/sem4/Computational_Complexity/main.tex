...
\begin{definition}
    \(\Sigma_k^p\) --- класс языков \(A\), таких, что \(\exists V\) --- полиномиально вычислимая, такая, что:
    \[x \in A \Lra \exists y_1\forall y_2 \dots^\forall_\exists y_k V(x, y_1, \dots y_k)\]
\end{definition}

\begin{definition}
    \(\Pi_k^p\) --- класс языков \(A\), таких, что \(\exists V\) --- полиномиально вычислимая, такая, что:
    \[x \in A \Lra \exists y_1\forall y_2 \dots^\forall_\exists y_k V(x, y_1, \dots y_k)\]
\end{definition}
...
\begin{definition}
    \[\mathbf{PH} = \bigcup_{k = 0}^\infty \Sigma_k^p = \bigcup_{k = 0}^\infty \Pi_k^p\]
\end{definition}
...
\begin{note}
    \(A, B \in \Sigma_k^p \Ra A \cup B, A \cap B \in \Sigma_k^p, A \in \Sigma_k^p \Lra \overline{A} \in \Pi_k^p\)
\end{note}
\begin{proof}
    \(A \in \Sigma_k^p \Lra \overline{A} \in \Pi_k^p\) --- очевидно из отрицания к формуле \(A\). Докажем, что \(A \cup B \in \Sigma_k^p\). Это следует из следующего (если \(\phi, \psi\) не зависят от \(y, x\) соответственно):
    \[\forall x \phi(x) \vee \forall y \psi(y) \lra \forall (x, y) \phi \vee \psi\]
    Утверждение \(A \cap B \in \Sigma_k^p\) получается и равенства \(A \cap B = \overline{\overline{A} \cup \overline{B}}\).
\end{proof}

\begin{theorem}[Об условиях коллапса]
    Следующие условия эквивалентны:
    \begin{enumerate}
        \item \(\Sigma_k^p = \Sigma_{k + 1}^p\)
        \item[1'.] \(\Pi_k^p = \Pi_{k + 1}^p\)
        \item \(\Sigma_k^p = \Pi_k^p, k > 0\)
        \item \(\mathbf{PH} = \Sigma_k^p\)
    \end{enumerate}
\end{theorem}
\begin{proof}\indent
    \begin{enumerate}
        \item[\((3) \Ra (1)\)] \(\Sigma_k^p \subset \Sigma_{k + 1}^p \subset \mathbf{PH} = \Sigma_k^p\)
        \item[\((1) \Ra (2)\)] \(\Pi_k^p \subset \Sigma_{k + 1}^p = \Sigma_k^p \Ra \Pi_k^p = \Sigma_k^p\). Теперь:
        \[A \in \Sigma_k^p \Ra \overline{A} \in \Pi_k^p \Ra \overline{A} \in \Sigma_k^p \Ra A \in \Pi_k^p \Ra \Sigma_k^p = \Pi_k^p\]
        ...
        \[Z = \{(x, y_1): \forall y_2 \dots_\exists^\forall y_{k + 1} V(x, y_1, \dots y_{k + 1})\} \in \Pi_k^p \Ra \in \Sigma_k^p\]
        Но тогда:
        ...
        Получили, что \(A \in \Sigma_k^p\).
        Тогда \(\Sigma_{k + 1}^p = \Pi_{k + 1}^p = \Sigma_k^p\). Отсюда по индукции получаем, что \(\Sigma_{k + l} = \Pi_{k + l} = \Sigma_k^p \Ra \mathbf{PH} = \Sigma_k\)
    \end{enumerate}
\end{proof}

\begin{note}
    \(A \le_p B, B \in \Sigma_k^p \Ra A \in \Sigma_k^p\).
\end{note}

\begin{note}
    Можно по аналогии определить \(\Sigma_k^p\)-полные, \(\Pi_k^p\)-полные и \(\mathbf{PH}\)-полные задачи.
\end{note}

\begin{theorem}
    \(\forall k \exists \Sigma_k^p\)-полные и \(\Pi_k^p\)-полные задачи. \(\mathbf{PH}\)-полные задачи существуют в том и только том случае, когда иерархия коллапсирует.
\end{theorem}
