...
% --- Row 1, Column 1: Устойчивый узел (stable node) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Устойчивый узел},
    xlabel={$x$}, ylabel={$y$},
    axis lines = center,
...
\pgfplotsinvokeforeach{0,30,60,90,120,150,180,210,240,270,300,330}{
    \addplot[domain=0:2, samples=50, variable=\t, ->, thick, -{Stealth}] 
        ({2*cos(#1)*exp(-t)}, {2*sin(#1)*exp(-3*t)});
}
\end{axis}
\end{tikzpicture}
\end{minipage}%
% --- Row 1, Column 2: Неустойчивый узел (unstable node) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
...
\pgfplotsinvokeforeach{0,30,60,90,120,150,180,210,240,270,300,330}{
    \addplot[domain=0:0.7, samples=50, variable=\t, ->, thick, -{Stealth}] 
        ({0.5*cos(#1)*exp(t)}, {0.5*sin(#1)*exp(2*t)});
}
\end{axis}
\end{tikzpicture}
\end{minipage}%
% --- Row 1, Column 3: Седло (saddle) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
...
\foreach \x in {-2,-1,1,2}{
    \addplot[domain=-1:1, samples=50, variable=\t, ->, thick, -{Stealth}] 
        ({\x*exp(-t)}, {0});
}
...
% --- Row 2, Column 1: Диакритический узел (star node) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Диакритический узел},
    xlabel={$x$}, ylabel={$y$},
    axis lines = center,
    xmin=-3, xmax=3, ymin=-3, ymax=3,
    grid = major,
...
\pgfplotsinvokeforeach{0,30,60,90,120,150,180,210,240,270,300,330}{
    \addplot[domain=0:2, samples=50, variable=\t, ->, thick, -{Stealth}] 
        ({2*cos(#1)*exp(-t)}, {2*sin(#1)*exp(-t)});
}
\end{axis}
\end{tikzpicture}
\end{minipage}%
% --- Row 2, Column 2: Вырожденный узел (degenerate node, one eigenvector) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Один собственный вектор},
    xlabel={$x$}, ylabel={$y$},
    axis lines = center,
    xmin=-3, xmax=3, ymin=-3, ymax=3,
    grid = major,
...
\pgfplotsinvokeforeach{0,30,60,90,120,150,180,210,240,270,300,330}{
    \addplot[domain=0:3, samples=70, variable=\t, ->, thick, -{Stealth}] 
        ({ (2*cos(#1) + 2*sin(#1)*t)*exp(-t) }, { 2*sin(#1)*exp(-t) });
}
\end{axis}
\end{tikzpicture}
\end{minipage}%
% --- Row 2, Column 3: Комплексные собственные числа (spiral) ---
\begin{minipage}{0.33\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Комплексные с.ч.},
    xlabel={$x$}, ylabel={$y$},
    axis lines = center,
    xmin=-3, xmax=3, ymin=-3, ymax=3,
    grid = major,
...
\pgfplotsinvokeforeach{0,30,60,90,120,150,180,210,240,270,300,330}{
    \addplot[domain=0:4*pi, samples=200, variable=\t, ->, thick, -{Stealth}] 
        ({2*exp(-0.2*t)*cos(t + #1*pi/180)}, {2*exp(-0.2*t)*sin(t + #1*pi/180)});
}
\end{axis}
\end{tikzpicture}
\end{minipage}

\caption{Фазовые портреты линейных автономных систем для различных типов собственных чисел.}
\end{figure}
